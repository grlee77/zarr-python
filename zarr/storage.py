...
def _normalize_store_arg_v2(store: Any, storage_options=None, mode="r") -> BaseStore:
    ...
    if zarr_version != 2:
        raise ValueError(f"store must be a version 2 store")
    if store is None:
        store = KVStore(dict())
        return store
...
def normalize_store_arg(store: Any, storage_options=None, mode="r", *,
    ...
    # if zarr_version == 3 and not v3_enabled:
    #     raise ValueError("Please set environment variable "
    #                      "ZARR_ENABLE_V3=true for Zarr v3 support")
    elif zarr_version not in [2, 3]:
        raise ValueError(f"zarr_version must be either 2 or 3")
    if zarr_version == 2:
